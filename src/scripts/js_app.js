...
jQuery(function ($) {
    ...
    _functions.initSwiper = function (el) {
        ...
        if (swiper.isLocked) {
            swiper.$el[0].classList.add("swiper-locked");
        }
        swiper.on("lock", function () {
            this.$el[0].classList.add("swiper-locked");
        });
        swiper.on("unlock", function () {
            this.$el[0].classList.remove("swiper-locked");
        });
    };
...
function animation() {
    if ($('.animate-scroll').length) {
        $('.animate-scroll').not('.animated').each(function () {
            let th = $(this);
